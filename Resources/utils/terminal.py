try:
  ...
  from Resources.utils.design import tascii1, skull
  from Resources.utils.handling import handler
  from Resources.expdb.dos import doser # Forced to import via full file destination because __init__ file only sends Resources to everything above utils and expdb.
  from Resources.expdb.tokentools import unverify, check_one, nitro, tpass
    ...
    def parse(self):
        if self.command == "clear":
          ...
          try:
            ...
            print(f"{coloring.RED}「Command Missing Arguments」{coloring.RESET}")
  ...
  State/Province : {g}
          ...
          if "-e vcdos" == self.command[0:8]:
            ...
            if "." in ip:
              ...
              print(f"{coloring.RED}「Invalid IPV4 Inputted」{coloring.RESET}")
          elif "-e tpass" == self.command[0:8]:
            email = input(f"{coloring.BLUE} ┗> Enter Email: ")
            password = getpass.getpass(prompt=f'{coloring.BLUE} ┗> Enter Password:')
            ...
            try:
              ...
              decis = str(input(f'{coloring.BLUE} ┗> Enter G to get messages or S to send: '))
              if decis.lower() == "s":
                cont = str(input(f'{coloring.BLUE} ┗> Enter message content: '))
